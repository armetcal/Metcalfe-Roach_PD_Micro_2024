...
```{r Test the validity of the wilcoxon signed-rank test}
# Symmetrical distribution for differences
valid_bw = bw %>% dplyr::select(Status,Species,Betweenness) %>% 
  pivot_wider(names_from = Status, values_from = Betweenness) %>% 
  mutate(diff = PD-Ctrl)
# Histogram
v1 = valid_bw %>% ggplot(aes(diff)) +
  geom_density() +
  theme_classic(base_size=18) +
  xlab('Diff. Betweenness') + ylab('Number of Species')

valid_cl = close %>% dplyr::select(Status,Species, Closeness) %>%
  pivot_wider(names_from = Status, values_from = Closeness) %>% 
  mutate(diff = PD-Ctrl)
# Histogram
v2 = valid_cl %>% ggplot(aes(diff)) +
  geom_density() +
  theme_classic(base_size=18) +
  xlab('Diff. Closeness') + ylab('Number of Species')

valid_dis = dist_all.noinf %>% dplyr::select(Status,Comparison,Distance) %>% 
  pivot_wider(names_from = Status, values_from = Distance) %>% 
  mutate(diff = PD-Ctrl)
# Histogram
v3 = valid_dis %>% ggplot(aes(diff)) +
  geom_density() +
  theme_classic(base_size=18) +
  xlab('Diff. Distance between\neach taxonomic pair') + ylab('Number of taxonomic pairs')

valid = ggarrange(plotlist = list(v1,v2,v3),ncol=3)
valid

ggsave("../Results/Supplementary Data/Validation of Statistical Tests/Figure 1 - Symmetrical differences for paired Wilcoxon tests.jpeg",height=4, width=12)
```
...
dist_plot = dist %>% group_by(Status,Connected) %>% tally() %>% 
  ...
  # Add p value
  geom_segment(x = 0.8,xend = 2.2,y=9000,yend=9000, size = 0.8) +
  # p value is from 'chi' object
  annotate("text", x=1.5, y=9500, label="<0.001", color = "black",size=8) +
  ylim(0,9500) +
...
bw %>% arrange(Status) %>% group_by(Taxon) %>% 
  summarize(bw = Betweenness[1L] - Betweenness[2L]) %>% 
  summarize(bw = mean(bw))
...
# Medians
close.noinf %>% group_by(Status) %>% summarize(cl = median(Closeness))
close.noinf %>% arrange(Status) %>% group_by(Taxon) %>% 
  summarize(bw = Closeness[1L] - Closeness[2L]) %>% 
  summarize(bw = mean(bw))

# Below 0.001, manually annotate p value on plot
close.wilcox$Value[close.wilcox$Parameter=='p.value']
...
close_plot = close.noinf %>% 
  ...
  theme_classic(base_size = 22) +
  xlab(NULL) + ylab('Closeness Centrality (Log 10)') +
  geom_line(aes(group = Species)) +
  ylim(-1.25,0.2) +
  theme(legend.position = 'none') +
  # Add p value
  geom_segment(x = 1,xend = 2,y=0.05,yend=0.05, size = 0.8) +
  # p value is from 'chi' object
  annotate("text", x=1.5, y=0.13, label="<0.001", color = "black",size=8) +
  scale_fill_grey(start = 0.3, end=0.7)
```
...
# Medians
dist_all.noinf %>% group_by(Status) %>% summarize(cl = mean(Distance))
dist_all.noinf %>% arrange(Status) %>% group_by(Comparison) %>% 
  summarize(bw = Distance[1L] - Distance[2L]) %>% 
  summarize(bw = mean(bw))
...
dist_all_plot = dist_all.noinf %>% 
  ...
  theme(legend.position = 'none') +
  # Add p value
  geom_segment(x = 1,xend = 2,y=13,yend=13, size = 0.8) +
  # p value is from 'chi' object
  annotate("text", x=1.5, y=14, label="<0.001", color = "black",size=8) +
  ylim(0,14) +
  scale_fill_grey(start = 0.3, end=0.7)
dist_all_plot
```
...
```{r}
plot_grid(dist_plot, bw_plot, close_plot, dist_all_plot)+
  theme(plot.margin = margin(1,0.1,0.1,2, "cm")) +
  draw_label(label="A",x=-0.015,y=1, fontface = "bold", size = 40) +
  draw_label(label="C",x=-0.015,y=0.5, fontface = "bold", size = 40) +
  draw_label(label="B",x=0.47,y=1, fontface = "bold", size = 40) +
  draw_label(label="D",x=0.47,y=0.5, fontface = "bold", size = 40)
ggsave('../Results/Figures/Figure 1 - Microbial Network Analysis.pdf', height = 9, width = 12.5)
```
...
```{r Combine}
plot_grid(net_plot, s1_plot, ncol = 2, rel_widths = c(3.3,2))+
  theme(plot.margin = margin(1,0.1,0.1,2, "cm")) +
  draw_label(label="A",x=-0.015,y=1, fontface = "bold", size = 40) +
  draw_label(label="B",x=0.62,y=1, fontface = "bold", size = 40)

ggsave('../Results/Supp Figures/Figure S1 - Network and Betweenness Centrality Phylum.pdf', height = 5, width = 16)
```
