...
```{r mean follow up}
temp=long %>% filter(!is.na(Status),!is.na(months_last_visit)) %>% 
  filter(Status=='PD') %>% 
  arrange(months_last_visit) %>% 
  group_by(Sample) %>%
  slice(n())

mean(temp$months_last_visit)
sd(temp$months_last_visit)
```
...
tempstats$Variable[tempstats$Variable=='mds3.total.nona'] = 'Mod. 3 '
tempstats$Variable[tempstats$Variable=='mds.total.nona'] = 'Total Mod.'
tempstats$Species = str_remove_all(tempstats$Species,'s__') %>% str_replace_all('_',' ')
...
ggsave('../Results/Supplementary Figures/Fig. 4a - Species vs Disease Progression - Univar.png',height=6,width=8)
```
...
ggsave('../Results/4. Progression/Fig. 4a - Species vs Disease Progression - Multivar.png',height=6,width=8)
```
...
writexl::write_xlsx(L,'../Results/Supplementary Data/Progression/Species vs Disease Progression Stats.xlsx')
```
