...
count_table = df6 %>% 
  mutate(`MetaCyc (%)` = round(n/nrow(sig_multi),2)) %>% 
  rename(Taxon = taxon, `MetaCyc (/7)` = n) %>% 
  mutate(Taxon = sapply(Taxon, function(x){
    if(x!='unclassified') x = x %>% str_split('[.]') %>% .[[1]] %>% .[2]
    return(x)
  })) 
...
test %>% 
  ...
  ylab('Prop. MetaCyc Terms') + xlab('Min Prop. Counts')

ggsave('../Results/Supplementary Figures/Key_MetaCyc_Species.jpeg',height=4.5, width=6.5)
```
