...
count_table = df6 %>% 
  mutate(`COG (%)` = round(n/nrow(sig_multi),2)) %>% 
  rename(Taxon = taxon, `COG (/38)` = n) %>% 
  mutate(Taxon = sapply(Taxon, function(x){
    if(x!='unclassified') x = x %>% str_split('[.]') %>% .[[1]] %>% .[2]
    return(x)
  })) 
...
writexl::write_xlsx(list('Avg Proportion' = avg_prop,
                    ...
                    '../Results/Supplementary Data/Differential Abundance/Stratified COG.xlsx')
```
...
test %>% 
  ...
  ylab('Prop. COG Terms') + xlab('Min Prop. Counts')

ggsave('../Results/Supplementary Figures/Key_COG_Species.jpeg',height=4.5, width=6.5)
```
