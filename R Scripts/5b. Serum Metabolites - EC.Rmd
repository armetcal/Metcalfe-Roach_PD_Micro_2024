...
brite$ec = sapply(brite$ec,function(x)str_split(x,' // ') %>% .[[1]] %>% .[1])

# Select species and melt phyloseq object
psm = ps %>% subset_taxa(Species %in% sig) %>% psmelt() %>%
  select(Species,everything()) %>% select(-Species) %>% 
  separate(col=OTU, into = c("Species","desc"), sep=': ') %>% select(-desc) %>%
  pivot_wider(names_from = Species, values_from = Abundance)

# Combine the datasets
psm = psm %>% left_join(met)

# Change sig vector to remove no_name labels
sig = sapply(sig,function(x) str_split(x,': ') %>% .[[1]] %>% .[1]) %>% unname
...
spearman.lax = correlate_met(temp, 'pcresol',grp='All') %>% 
  ...
  left_join(brite %>% rename(taxon=EC) %>% select(taxon,ec)) %>% select(-taxon) %>% rename(taxon=ec)

# Extract just the relevant rows for plotting
spearman.lax = spearman.lax %>% unique
```
...
spearman.lax$taxon = factor(spearman.lax$taxon, levels = unique(species_order$taxon) )

# Plot amino acids
p1 = prep_plot(c('Phenylacetylglutamine','p-Cresol'))

ggsave('../Results/Supplementary Figures/Fig. 3cd - EC vs Metabolites - Univar.png',height=17,width=6)
```
...
glm_species$Taxon = factor(glm_species$Taxon, levels = unique(species_order$taxon) )

# Plot amino acids
p1 = prep_plot(c('Phenylacetylglutamine','p-Cresol'))

ggsave('../Results/Supplementary Figures/Fig. 3cd - EC vs Metabolites - Multivar.png',height=17,width=6)
```
...
writexl::write_xlsx(list('Univar' = spearman.lax,
                    ...
                    '../Results/Supplementary Data/Metabolite Associations/EC vs Metabolites.xlsx')
```