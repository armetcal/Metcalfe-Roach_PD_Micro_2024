---
title: "6c. Disease Progression - COG"
...
psm = ps %>% subset_taxa(Species %in% sig) %>% psmelt() %>%
  ...
  separate(col=OTU, into = c("COG","desc"), sep=': ')
...
pd = long %>% filter(Status == 'PD') %>% group_by(COG) %>% 
...
ggsave('../Results/Supplementary Figures/Fig. 4 - COG vs Disease Progression - Univar.png',height=18,width=9)
```
...
ggsave('../Results/Supplementary Figures/Fig. 4 - COG vs Disease Progression - Multivar.png',height=18,width=9)
```
...
writexl::write_xlsx(L,'../Results/Supplementary Data/Progression/COG vs Disease Progression Stats.xlsx')
```
