...
order = glm_species %>% filter(Response=='mds.total') %>% 
  arrange(Estimate) %>% 
  separate(col=Taxon, sep=': ',into=c('Taxon','desc')) %>% select(-desc) %>% 
  left_join(brite %>% select(EC,ec) %>% rename(Taxon=EC) %>% unique) %>% 
  mutate(ec = sapply(.$ec,function(x)str_split(x,' // ') %>% .[[1]] %>% .[1]))
```
...
prep_plot = function(vars){
  p = spearman %>% 
    ...
    ggplot(aes(variable,ec,col = rho)) +
    geom_point(size =14) +
...
spearman$ec = factor(spearman$ec, levels = unique(order$ec))
...
prep_plot = function(vars){
  p = glm_species %>% 
    ...
    ggplot(aes(Response,ec,col = Estimate)) +
    geom_point(size =14) +
    theme_classic(base_size = 16) +
    scale_color_gradient2(low="darkblue", high="darkred", guide="colorbar") +
    geom_text(aes(label=Qval),size = 10, col = 'white',nudge_y = -0.25) +
...
glm_species = glm_species %>% 
  separate(col=Taxon, sep=': ',into=c('Taxon','desc')) %>% select(-desc) %>% 
  left_join(brite %>% select(EC,ec) %>% rename(Taxon=EC) %>% unique) %>% 
  mutate(ec = sapply(.$ec,function(x)str_split(x,' // ') %>% .[[1]] %>% .[1]))

glm_species$ec = factor(glm_species$ec, levels = unique(order$ec))
...
ggsave('../Results/Supplementary Figures/Fig. 4d - EC vs Disease Severity - Univar.png',height=18,width=7)

# Multivar
ggarrange(plotlist = list(p))
ggsave('../Results/Supplementary Figures/Fig. 4d - EC vs Disease Severity - Multivar.png',height=18,width=7)
```
...
writexl::write_xlsx(list('Univar' = spearman,
                    ...
                    '../Results/Supplementary Data/Progression/EC vs Baseline Severity Stats.xlsx')
```