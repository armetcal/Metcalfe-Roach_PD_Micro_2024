...
avg_prop = df4 %>% 
  ...
  mutate(Feature = sapply(.$Feature,function(x) x %>% str_split(':') %>% 
                            .[[1]] %>% .[1] %>% str_remove('-RXN'))) %>% 
  mutate(Feature = sapply(.$Feature, function(x){
    # x='PECTINESTERASE'
    if(x %in% brite$EC2){
      return(brite$ec[(which(x==brite$EC2))[1]])
    } else {return(x)}
    }))

count_table = df6 %>% 
  mutate(`EC (%)` = round(n/nrow(sig_multi),2)) %>% 
  rename(Taxon = taxon, `EC (/44)` = n) %>% 
  mutate(Taxon = sapply(Taxon, function(x){
    if(x!='unclassified') x = x %>% str_split('[.]') %>% .[[1]] %>% .[2]
    return(x)
  })) 
...
test %>% 
  ...
  ylab('Prop. EC Terms') + xlab('Min Prop. Counts')

ggsave('../Results/Supplementary Figures/Key_EC_Species.jpeg',height=4.5, width=6.5)
```
